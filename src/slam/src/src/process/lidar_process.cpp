...
void Preprocess::avia_handler(const pcl::PointCloud<livox_pcl::Point>& msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
    ...
    if (feature_enabled)
    {
        for (uint i = 1; i < plsize; i++)
        {
            if ((msg.points[i].line < N_SCANS) && ((msg.points[i].tag & 0x30) == 0x10 || (msg.points[i].tag & 0x30) == 0x00))
            {
                ...
                pl_full[i].intensity = msg.points[i].intensity;
        ...
        for (int j = 0; j < N_SCANS; j++)
        {
            if (pl_buff[j].size() <= 5)
                continue;
            pcl::PointCloud<robot::slam::PointType>& pl = pl_buff[j];
            plsize                                      = pl.size();
        ...
        for (uint i = 1; i < plsize; i++)
        {
            if ((msg.points[i].line < N_SCANS) && ((msg.points[i].tag & 0x30) == 0x10 || (msg.points[i].tag & 0x30) == 0x00))
            {
                ...
                if (valid_num % point_filter_num == 0)
                {
                    ...
                    pl_full[i].intensity = msg.points[i].intensity;
                    pl_full[i].curvature =
                        msg.points[i].timestamp / float(1000000);  // use curvature as time of each laser points, curvature unit: ms
...
void Preprocess::give_feature(pcl::PointCloud<robot::slam::PointType>& pl, vector<orgtype>& types)
{
    int plsize = pl.size();
    int plsize2;
    if (plsize == 0)
...
int Preprocess::plane_judge(
    const robot::slam::PointCloudType& pl, vector<orgtype>& types, uint i_cur, uint& i_nex, Eigen::Vector3d& curr_direct)
{
...
bool Preprocess::edge_jump_judge(const robot::slam::PointCloudType& pl, vector<orgtype>& types, uint i, Surround nor_dir)
{
    if (nor_dir == 0)
    {
