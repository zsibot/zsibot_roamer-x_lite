...
class BaseTF {
 ...
 protected:
  ...
  /**
   * @brief Publishes joint state information derived from the navigation state
   * message.
   *
   * This function extracts the joint positions, velocities, and names from the
   * provided NavigationState message and populates a JointState message. It
   * then publishes the constructed JointState message for use in downstream
   * components.
   *
   * @param state_msg NavigationState message containing joint data such as
  ...
  /**
   * @brief Handles incoming joint state messages to process joint positions and
   * publishes low-level commands.
   *
   * This callback function validates and processes the received
   * `sensor_msgs::msg::JointState` message. It maps joint names to their
   * respective positions and ensures the consistency of the data. It prepares a
   * low-level command message to update actuator targets, which are then
   * published to the corresponding topic.
   *
   * If the `name` and `position` vector sizes in the joint state message
   * mismatch, or if there are invalid or duplicate joint names, warnings or
   * errors will be logged, and the processing will terminate for that instance.
   *
   * The method adheres to a predefined order of joint names, `kOrder`, to
   * generate the low-level command message.
   *
   * Uses the `low_level_cmd_pub_ptr_` publisher to send the generated low-level
   * command to the appropriate topic.
   *
   * Logs errors if the publisher is unavailable or if any anomalies are
   * detected in the received message.
   *
   * @param state_msg The `sensor_msgs::msg::JointState` message containing
 ...
 protected:
  ...
  rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr
      joint_states_pub_ptr_;

  rclcpp::Subscription<sensor_msgs::msg::JointState>::SharedPtr
      joint_states_cmd_sub_ptr_;
  rclcpp::Publisher<mc_sdk_msg::msg::LowLevelCmd>::SharedPtr
      low_level_cmd_pub_ptr_;
  ...
  std::unordered_map<std::string, std::pair<std::string, std::string>>
      platform_map_{{"NX_XG3588", {"192.168.3.120", "192.168.3.100"}},
