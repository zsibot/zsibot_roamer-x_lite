...
#include <hdl_localization/mode_state.h>
...
public:
  ...
  HdlLocalizationNode(const rclcpp::NodeOptions& options) : Node("localization", options) {
    ...
    // Frame for odometry child frame published by localization
    ...
    RCLCPP_INFO(get_logger(),
                ...
                use_global_localization ? "true" : "false",
                reg_method.c_str(),
    ...
    // Test quaternion conversion function
...
private:
  ...
  void initialize_params() {
    ...
    double downsample_resolution = declare_parameter<double>("downsample_resolution", 0.2);
    std::shared_ptr<pcl::VoxelGrid<PointT>> voxelgrid(new pcl::VoxelGrid<PointT>());
    // TODO 
    voxelgrid->setLeafSize(points_voxel_filter_size_, points_voxel_filter_size_, points_voxel_filter_size_);
...
private:
  void imu_callback(const sensor_msgs::msg::Imu::SharedPtr imu_msg) {
    ...
    // Update IMU data validity status
  ...
  void points_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr points_msg) {
    ...
    if (!isSensorDataValid()) {
      ...
      // Use extrapolation logic to publish localization information
      publishExtrapolatedOdom(points_msg->header.stamp);
      return;
    }

    // Add sensor status debug information
    RCLCPP_DEBUG_THROTTLE(get_logger(), *get_clock(), 5.0, 
                          "Sensor status - Lidar: %s, IMU: %s, Overall: %s", 
                          isLidarDataValid() ? "Valid" : "Invalid",
                          isImuDataValid() ? "Valid" : "Invalid",
                          isSensorDataValid() ? "Valid" : "Invalid");
    ...
    if (!is_init_success_) {
      ...
      if (init_result.is_converged_ && init_result.fitness_score_ < 0.2) {
        is_init_success_ = true;
        localization_state_ = 2;
        RCLCPP_INFO(get_logger(), "Init Pose Successful!!!");
      } else {
        RCLCPP_INFO(get_logger(), "Init Pose Failed!!!");
      }
    }
    ...
    if (last_timeout_ > 80) {
      RCLCPP_INFO(get_logger(), "!!!point cloud callback time cost > %d ms\n", last_timeout_);
    }
    ...
    if (match_result.is_converged_ && match_result.fitness_score_ < 0.5) {
      // When localization succeeds, update pose history
      ...
      // When localization succeeds, reset extrapolation state, confidence recovers to 1.0
      is_extrapolating_ = false;
  ...
  void publish_odometry(const rclcpp::Time& stamp, const Eigen::Matrix4f& pose) {
    ...
    // Set confidence to the first element of covariance matrix
  ...
  void pubDefaultLocalizationOdom(const rclcpp::Time& stamp) {
    // Set extrapolation state to false, confidence to 0
    is_extrapolating_ = false;
    current_confidence_ = 0.0;
    
    // Create default transformation matrix (position is 0, orientation is identity matrix)
  ...
  bool isLidarDataValid() {
    // Count valid data in recent N times
    if (lidar_status_buffer_.size() < min_valid_count_) {
      return false; // Insufficient data, considered invalid
    }
    
    int valid_count = std::count(lidar_status_buffer_.begin(), 
                                 lidar_status_buffer_.end(), true);
  ...
  bool isImuDataValid() {
    if (!use_imu) {
      return false; // If IMU is not enabled, return false directly
    }
    
    // Count valid data in recent N times
    if (imu_status_buffer_.size() < min_valid_count_) {
      return false; // Insufficient data, considered invalid
    }
    
    int valid_count = std::count(imu_status_buffer_.begin(), 
                                 imu_status_buffer_.end(), true);
    return valid_count >= min_valid_count_;
  }
  ...
  bool isSensorDataValid() {
    return isLidarDataValid() || isImuDataValid();
  }
  ...
  Eigen::Vector3f getCurrentVelocity(const Eigen::Matrix4f& current_pose, const rclcpp::Time& current_time) {
    // Prioritize UKF state
    if (pose_estimator) {
      return pose_estimator->vel();
    }
    
    // If UKF is not available, use extrapolation to calculate velocity
    if (has_valid_pose_history_) {
      ...
      if (dt > 0.0) {
        ...
        // Calculate velocity v = Δs / Δt
        return position_delta / dt;
      }
    }
    
    // If none available, return last velocity or zero velocity
  ...
  Eigen::Vector3f getCurrentAngularVelocity(const Eigen::Matrix4f& current_pose, const rclcpp::Time& current_time) {
    ...
    // If IMU is not available, use extrapolation to calculate angular velocity
    if (has_valid_pose_history_) {
      double dt = (current_time - last_valid_pose_time_).seconds();
      if (dt > 0.0) {
        ...
        // Convert rotation matrix to angular velocity (using logarithmic mapping)
    ...
    // If none available, return last angular velocity or zero angular velocity
  ...
  void updateConfidence(const rclcpp::Time& current_time) {
    if (!is_init_success_) {
      // Before initialization, confidence stays at 0.0
      current_confidence_ = 0.0;
      ...
      if (dt > 0.0) {
        // Exponential decay: confidence = confidence * exp(-decay_rate * dt)
        current_confidence_ *= std::exp(-confidence_decay_rate_ * dt);
        
        // Limit minimum confidence
        if (current_confidence_ < 0.01) {
          current_confidence_ = 0.01;
        }
      }
    } else {
      // During normal localization, confidence recovers to 1.0
      current_confidence_ = 1.0;
    }
  ...
  Eigen::Vector3f quaternionToNormalizedRPY(const Eigen::Quaternionf& quat) {
    ...
    // Use mathematical formulas for ZYX order (consistent with Eigen::eulerAngles(0,1,2))
    // 0=Z-axis(yaw), 1=Y-axis(pitch), 2=X-axis(roll)
    
    float roll, pitch, yaw;
    
    // Calculate Roll (around X-axis, 3rd rotation)
    float sinr_cosp = 2.0f * (w * x + y * z);
    float cosr_cosp = 1.0f - 2.0f * (x * x + y * y);
    roll = std::atan2(sinr_cosp, cosr_cosp);
    
    // Calculate Pitch (around Y-axis, 2nd rotation)
    float sinp = 2.0f * (w * y - x * z);
    if (std::abs(sinp) >= 1.0f) {
      // Handle gimbal lock case (pitch = ±90°)
      pitch = std::copysign(M_PI / 2.0f, sinp);
      // During gimbal lock, roll and yaw cannot be distinguished, use special handling
      roll = 0.0f;
      yaw = 2.0f * std::atan2(x, w);
    } else {
      pitch = std::asin(sinp);
      
      // Calculate Yaw (around Z-axis, 1st rotation)
      float siny_cosp = 2.0f * (w * z + x * y);
      float cosy_cosp = 1.0f - 2.0f * (y * y + z * z);
      yaw = std::atan2(siny_cosp, cosy_cosp);
    }
    
    // Normalize angle ranges (ZYX order)
    // Roll (X-axis): [-π/2, π/2]
    if (roll > M_PI / 2.0f) {
      roll -= M_PI;
      pitch = M_PI - pitch;
    ...
    // Return [roll, pitch, yaw] corresponding to [X-axis, Y-axis, Z-axis]
    return Eigen::Vector3f(roll, pitch, yaw);
  }

  /**
   * @brief Test quaternion conversion (for debugging)
   */
  // void testQuaternionConversion() {
  //   // Test with the quaternion data you provided
  //   Eigen::Quaternionf test_quat(0.9670712091421966f, -0.029939700464583958f, 
  //                               -0.08820705738705632f, 0.23684662472198365f);
    
  //   RCLCPP_INFO(get_logger(), "=== Quaternion Conversion Test ===");
  //   RCLCPP_INFO(get_logger(), "Input quaternion: w=%.6f, x=%.6f, y=%.6f, z=%.6f", 
  //                test_quat.w(), test_quat.x(), test_quat.y(), test_quat.z());
    
  //   // Test our function
  //   Eigen::Vector3f rpy_ours = quaternionToNormalizedRPY(test_quat);
  //   RCLCPP_INFO(get_logger(), "Our conversion: roll=%.6f, pitch=%.6f, yaw=%.6f", 
  //                rpy_ours(0), rpy_ours(1), rpy_ours(2));
    
  //   // Test Eigen's function for comparison
  //   Eigen::Vector3f rpy_eigen = test_quat.toRotationMatrix().eulerAngles(0, 1, 2);
  //   RCLCPP_INFO(get_logger(), "Eigen conversion: roll=%.6f, pitch=%.6f, yaw=%.6f", 
  //                rpy_eigen(0), rpy_eigen(1), rpy_eigen(2));
    
  //   RCLCPP_INFO(get_logger(), "=== End Test ===");
  // }
  ...
  Eigen::Matrix4f extrapolatePose(const rclcpp::Time& current_time) {
    if (!has_valid_pose_history_) {
      // No historical data, return identity matrix
    ...
    // Velocity and angular velocity range limits
    ...
    // Position extrapolation: s = s0 + v * dt
    Eigen::Vector3f position_delta = limited_velocity * dt;
    
    // Extract position and attitude from previous pose
    Eigen::Vector3f last_position = last_pose_.block<3, 1>(0, 3);
    Eigen::Matrix3f last_rotation = last_pose_.block<3, 3>(0, 0);
    ...
    // Attitude extrapolation: use angular velocity integration
    // For small angle changes, linear approximation can be used
    Eigen::Vector3f angle_delta = limited_angular_velocity * dt;
    
    // Create rotation matrix for angle changes (using Euler angle approximation)
  ...
  void publishExtrapolatedOdom(const rclcpp::Time& stamp) {
    if (!has_valid_pose_history_) {
      // No historical data, publish default localization information
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1.0, 
    ...
    // Perform pose extrapolation
    Eigen::Matrix4f extrapolated_pose = extrapolatePose(stamp);
    
    // Publish extrapolated localization information
    publish_odometry(stamp, extrapolated_pose);
    
    // Key: Update pose history to enable continuous extrapolation
    // Use extrapolated pose as new historical pose
  ...
  void PublishLidarLocalizationInfo() {
        ...
        if (lidar_status_buffer_.size() > 0) {
            ...
            if (lidar_time_diff > sensor_timeout_threshold_) {
                // Lidar data timeout, mark as invalid
        ...
        if (!is_init_success_) {
            ...
            msg.speed = 0.0;
            // Ensure confidence reflects not initialized state
            current_confidence_ = 0.0;
        ...
        msg.speed = vel.norm();
        localization_info_pub_->publish(msg);
        // Output sensor status information (using frequency control)
        controlledLogInfo("Sensor Status: " + getSensorStatusInfo());
    }
    ...
    void PublishOdomTimer() {
        // If localization not initialized, publish default odometry
        if (!is_init_success_) {
        ...
        // If sensor data invalid, use extrapolation logic to publish odometry
        if (!isSensorDataValid()) {
            RCLCPP_DEBUG(get_logger(), "Sensor data invalid, checking pose history...");
            auto current_time = this->get_clock()->now();
            // As long as localization has succeeded and has pose history, use extrapolation logic
            if (has_valid_pose_history_) {
                ...
                // No pose history, use default localization
    ...
    void LoadMapCallBack(robots_dog_msgs::srv::LoadMap::Request::SharedPtr request, 
        ...
        RCLCPP_INFO(get_logger(), "Using lidar SLAM map for localization");
        ...
        if (!global_map_points_ptr_->empty()) {
            ...
            if (global_map_points_ptr_->points.size() < 1000) {
                ...
                if (global_map_pub_->get_subscription_count()) {
                    // Downsample separately before publishing
    ...
    void Reset()
    {
        ...
        // Reset sensor data validity status
...
private:
  ...
  rclcpp::TimerBase::SharedPtr odom_publish_timer_; // Add odometry publishing timer
  ...
  int min_valid_count_;      // Minimum number of valid data points required to consider sensor valid
  int buffer_size_;          // Buffer size
  double sensor_timeout_threshold_ = 1.0; // Sensor data timeout threshold (seconds)
  ...
  float init_y_ = 0.0;
  float init_z_ = 0.0;
  float init_ori_w_ = 1.0;
  float init_ori_x_ = 0.0;
  float init_ori_y_ = 0.0;
  float init_ori_z_ = 0.0;
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<hdl_localization::HdlLocalizationNode>(rclcpp::NodeOptions());
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
