...
public:
  ...
  // GlobalmapServerNode 생성자. 기본적으로 map_server 라는 이름을 갖는 인스턴스를 만듦
  : Node("map_server", options)
  {
    ...
    globalmap_pub_timer = create_wall_timer(std::chrono::milliseconds(1000), std::bind(&GlobalmapServerNode::pub_once_cb, this));
...
////***************************start pyc modified***************************/
    map_update_sub = create_subscription<std_msgs::msg::String>(
      "/map_request/pcd",
      latch_qos,
      std::bind(&GlobalmapServerNode::map_update_callback, this, std::placeholders::_1)
    );
/***************************end pyc modified***************************////
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<hdl_localization::GlobalmapServerNode>(rclcpp::NodeOptions());
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
