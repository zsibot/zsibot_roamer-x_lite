...
public:
        ...
        : Node("vel_cmd_udp_publisher")
    {
        planner_vel_cmd_subscriber = this->create_subscription<geometry_msgs::msg::Twist>(
            "/cmd_vel", 10, std::bind(&VelCmdUdpPublisher::HandlePlannerVelCallback, this, std::placeholders::_1));
        ...
        //             RCLCPP_INFO(this->get_logger(), "[%ld]==> 最终发送了: %f", now().nanoseconds(), cmd_.vx);
