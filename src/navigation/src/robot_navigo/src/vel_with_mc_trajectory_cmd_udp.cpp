...
public:
        ...
        : Node("vel_with_mc_trajectory_cmd_udp_publisher")
    {
        planner_vel_with_mc_traj_cmd_subscriber =
            this->create_subscription<robots_dog_msgs::msg::CmdVelWithTrajectory>("/cmd_vel_with_mc_trajectory", 10,
                std::bind(&VelWithMcTrajecotryCmdUdpPublisher::HandlPlannerVelWithMcTrajectoryCallback, this, std::placeholders::_1));
        RCLCPP_INFO(this->get_logger(), "vel_with_mc_trajectory_cmd_udp_publisher started");
        ...
        const std::unordered_map<std::string, std::pair<std::string, std::string>> platform_map = {
            { "NX_YSC3588", { "192.168.1.120", "192.168.1.100" } }, { "NX_XG3588", { "192.168.3.120", "192.168.3.100" } },
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "\033[1;31mError: Unknown PLATFORM type\033[0m");
            rclcpp::shutdown();
            return;
        }
        ...
        udp_process_thread_ = std::thread(
            [&]()
            {
                while (rclcpp::ok())
                {
                    ...
                    //    		    RCLCPP_INFO(this->get_logger(), "==> 最终发送了: %f",cmd_.vx);
...
private:
    void HandlPlannerVelWithMcTrajectoryCallback(const robots_dog_msgs::msg::CmdVelWithTrajectory::SharedPtr msg)
    {
        std::lock_guard<std::mutex> lk(planner_vel_mutex_);

        cmd_.vx       = std::fabs(msg->cmd_vel.twist.linear.x) < 0.085 ? 0.0 : msg->cmd_vel.twist.linear.x;
        cmd_.vy       = std::fabs(msg->cmd_vel.twist.linear.y) < 0.085 ? 0.0 : msg->cmd_vel.twist.linear.y;
        cmd_.yaw_rate = msg->cmd_vel.twist.angular.z;

        if (msg->trajectory.poses.size() != std::size(cmd_.trajectory))
        {
            RCLCPP_ERROR(this->get_logger(), "==> Received  traj size is not equal to mc reuqired trajectory size");
            RCLCPP_ERROR(this->get_logger(), "Received traj size : %ld | Mc trajectory : %ld", msg->trajectory.poses.size(),
                std::size(cmd_.trajectory));
            return;
        }
...
private:
    ...
    rclcpp::Subscription<robots_dog_msgs::msg::CmdVelWithTrajectory>::SharedPtr planner_vel_with_mc_traj_cmd_subscriber;
