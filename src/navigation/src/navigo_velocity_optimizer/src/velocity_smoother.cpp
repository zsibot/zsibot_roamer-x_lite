...
namespace navigo_velocity_optimizer
{
...
VelocityOptimizer::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  if (feedback_type == "OPEN_LOOP") {
    ...
    open_loop_ = false;
    odom_smoother_ = std::make_unique<navigo_util::OdomSmoother>(node, odom_duration_, odom_topic_);
  } else {
...
navigo_util::CallbackReturn
VelocityOptimizer::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  timer_ = this->create_wall_timer(
    ...
    std::bind(&VelocityOptimizer::smootherTimer, this));

  dyn_params_handler_ = this->add_on_set_parameters_callback(
    std::bind(&VelocityOptimizer::dynamicParametersCallback, this, _1));
...
navigo_util::CallbackReturn
VelocityOptimizer::on_deactivate(const rclcpp_lifecycle::State &)
{
...
navigo_util::CallbackReturn
VelocityOptimizer::on_cleanup(const rclcpp_lifecycle::State &)
{
...
navigo_util::CallbackReturn
VelocityOptimizer::on_shutdown(const rclcpp_lifecycle::State &)
{
...
double VelocityOptimizer::applyConstraints(
...
VelocityOptimizer::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters)
{
  ...
  for (auto parameter : parameters) {
    ...
    if (type == ParameterType::PARAMETER_DOUBLE) {
      if (name == "smoothing_frequency") {
        ...
        odom_smoother_ =
          std::make_unique<navigo_util::OdomSmoother>(
...
RCLCPP_COMPONENTS_REGISTER_NODE(navigo_velocity_optimizer::VelocityOptimizer)
