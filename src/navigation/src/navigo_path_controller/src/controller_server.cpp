...
: navigo_util::LifecycleNode("controller_server", "", options),
  progress_checker_loader_("navigo_core", "navigo_core::ProgressChecker"),
  default_progress_checker_id_{"progress_checker"},
  default_progress_checker_type_{"navigo_path_controller::SimpleProgressChecker"},
  goal_checker_loader_("navigo_core", "navigo_core::GoalChecker"),
  default_goal_checker_ids_{"goal_checker"},
  default_goal_checker_types_{"navigo_path_controller::SimpleGoalChecker"},
  lp_loader_("navigo_core", "navigo_core::Controller"),
...
ControllerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  costmap_thread_ = std::make_unique<navigo_util::NodeThread>(costmap_ros_);

  try {
    progress_checker_type_ = navigo_util::get_plugin_type_param(node, progress_checker_id_);
  ...
  for (size_t i = 0; i != goal_checker_ids_.size(); i++) {
    try {
      goal_checker_types_[i] = navigo_util::get_plugin_type_param(node, goal_checker_ids_[i]);
      navigo_core::GoalChecker::Ptr goal_checker =
  ...
  for (size_t i = 0; i != controller_ids_.size(); i++) {
    try {
      controller_types_[i] = navigo_util::get_plugin_type_param(node, controller_ids_[i]);
      navigo_core::Controller::Ptr controller =
...
void ControllerServer::computeControl()
{
  ...
  } catch (navigo_core::PlannerException & e) {
...
void ControllerServer::setPlannerPath(const nav_msgs::msg::Path & path)
{
  ...
  if (path.poses.empty()) {
    throw navigo_core::PlannerException("Invalid path, Path is empty.");
  }
...
void ControllerServer::computeAndPublishVelocity()
{
  ...
  try {
    ...
    if (failure_tolerance_ > 0 || failure_tolerance_ == -1.0) {
      ...
      {
        throw navigo_core::PlannerException("Controller patience exceeded");
      }
    } else {
...
RCLCPP_COMPONENTS_REGISTER_NODE(navigo_path_controller::ControllerServer)
