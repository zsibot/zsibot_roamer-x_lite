...
#include "navigo_behavior_tree/plugins/decorator/path_longer_on_approach.hpp"
...
bool PathLongerOnApproach::isRobotInGoalProximity(
  ...
  double & prox_leng)
{
  return navigo_util::geometry_utils::calculate_path_length(old_path, 0) < prox_leng;
}
...
{
  return navigo_util::geometry_utils::calculate_path_length(new_path, 0) >
         length_factor * navigo_util::geometry_utils::calculate_path_length(
    old_path, 0);
}
...
{
  factory.registerNodeType<navigo_behavior_tree::PathLongerOnApproach>("PathLongerOnApproach");
}
