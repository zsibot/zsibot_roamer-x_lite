...
#include "navigo_behavior_tree/plugins/decorator/speed_controller.hpp"
  ...
  max_speed_(0.5)
{
  ...
  odom_smoother_ = config().blackboard->get<std::shared_ptr<navigo_util::OdomSmoother>>(
    "odom_smoother");
}
...
{
  factory.registerNodeType<navigo_behavior_tree::SpeedController>("SpeedController");
}
