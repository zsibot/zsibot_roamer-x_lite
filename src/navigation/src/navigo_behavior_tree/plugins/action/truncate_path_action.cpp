...
#include "navigo_behavior_tree/plugins/action/truncate_path_action.hpp"
...
inline BT::NodeStatus TruncatePath::tick()
{
  ...
  double distance_to_goal = navigo_util::geometry_utils::euclidean_distance(
    input_path.poses.back(), final_pose);

  while (distance_to_goal < distance_ && input_path.poses.size() > 2) {
    input_path.poses.pop_back();
    distance_to_goal = navigo_util::geometry_utils::euclidean_distance(
      input_path.poses.back(), final_pose);
  }
  ...
  input_path.poses.back().pose.orientation = navigo_util::geometry_utils::orientationAroundZAxis(
    final_angle);
...
{
  factory.registerNodeType<navigo_behavior_tree::TruncatePath>("TruncatePath");
}
