...
#include "navigo_behavior_tree/plugins/action/truncate_path_local_action.hpp"
...
inline BT::NodeStatus TruncatePathLocal::tick()
{
  ...
  if (path_pruning) {
    closest_pose_detection_end = navigo_util::geometry_utils::first_after_integrated_distance(
      closest_pose_detection_begin_, path_.poses.end(), max_robot_pose_search_dist);
  }

  // find the closest pose on the path
  auto current_pose = navigo_util::geometry_utils::min_by(
  ...
  auto forward_pose_it = navigo_util::geometry_utils::first_after_integrated_distance(
  ...
  auto backward_pose_it = navigo_util::geometry_utils::first_after_integrated_distance(
...
BT_REGISTER_NODES(factory) {
  factory.registerNodeType<navigo_behavior_tree::TruncatePathLocal>(
    "TruncatePathLocal");
}
