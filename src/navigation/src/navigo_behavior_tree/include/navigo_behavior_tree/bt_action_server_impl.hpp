...
#ifndef NAVIGO_BEHAVIOR_TREE__BT_ACTION_SERVER_IMPL_HPP_
#define NAVIGO_BEHAVIOR_TREE__BT_ACTION_SERVER_IMPL_HPP_
...
bool BtActionServer<ActionT>::on_configure()
{
  ...
  navigo_util::copy_all_parameters(node, client_node_);
  ...
  bt_ = std::make_unique<navigo_behavior_tree::BehaviorTreeEngine>(plugin_lib_names_);
...
void BtActionServer<ActionT>::executeCallback()
{
  ...
  navigo_behavior_tree::BtStatus rc = bt_->run(&tree_, on_loop, is_canceling, bt_loop_duration_);
  ...
  switch (rc) {
    case navigo_behavior_tree::BtStatus::SUCCEEDED:
      action_server_->succeeded_current(result);
      RCLCPP_INFO(logger_, "Goal succeeded");
      break;

    case navigo_behavior_tree::BtStatus::FAILED:
      action_server_->terminate_current(result);
      RCLCPP_ERROR(logger_, "Goal failed");
      break;

    case navigo_behavior_tree::BtStatus::CANCELED:
...
#endif  // NAVIGO_BEHAVIOR_TREE__BT_ACTION_SERVER_IMPL_HPP_
