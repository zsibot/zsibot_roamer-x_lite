...
#include <cmath>
#include "navigo_mppi_controller/critics/obstacles_critic.hpp"
...
float ObstaclesCritic::findCircumscribedCost(
  std::shared_ptr<navigo_costmap_2d::Costmap2DROS> costmap)
{
  double result = -1.0;
  ...
  for (auto layer = costmap->getLayeredCostmap()->getPlugins()->begin();
    ...
    ++layer)
  {
    auto inflation_layer = std::dynamic_pointer_cast<navigo_costmap_2d::InflationLayer>(*layer);
    if (!inflation_layer) {
      continue;
    }
...
CollisionCost ObstaclesCritic::costAtPose(float x, float y, float theta)
{
  ...
  if (!collision_checker_.worldToMap(x, y, x_i, y_i)) {
    cost = navigo_costmap_2d::NO_INFORMATION;
