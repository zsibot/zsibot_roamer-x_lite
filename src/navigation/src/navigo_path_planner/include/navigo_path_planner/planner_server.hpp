...
#ifndef NAVIGO_PATH_PLANNER__PLANNER_SERVER_HPP_
#define NAVIGO_PATH_PLANNER__PLANNER_SERVER_HPP_
...
#include "navigo_core/global_planner.hpp"
#include "nav2_msgs/srv/is_path_valid.hpp"

namespace navigo_path_planner
{
/**
 * @class navigo_path_planner::PlannerServer
 ...
 */
class PlannerServer : public navigo_util::LifecycleNode
{
public:
  /**
   * @brief A constructor for navigo_path_planner::PlannerServer
   * @param options Additional options to control creation of the node.
   */
  explicit PlannerServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions());
  /**
   * @brief A destructor for navigo_path_planner::PlannerServer
   */
  ~PlannerServer();

  using PlannerMap = std::unordered_map<std::string, navigo_core::GlobalPlanner::Ptr>;
...
protected:
  ...
  using ActionServerToPose = navigo_util::SimpleActionServer<ActionToPose>;
  using ActionServerThroughPoses = navigo_util::SimpleActionServer<ActionThroughPoses>;
   ...
   */
  template<typename T>
  bool isServerInactive(std::unique_ptr<navigo_util::SimpleActionServer<T>> & action_server);
  ...
  pluginlib::ClassLoader<navigo_core::GlobalPlanner> gp_loader_;
  ...
  std::unique_ptr<navigo_util::NodeThread> costmap_thread_;
  navigo_costmap_2d::Costmap2D * costmap_;
...
#endif  // NAVIGO_PATH_PLANNER__PLANNER_SERVER_HPP_
