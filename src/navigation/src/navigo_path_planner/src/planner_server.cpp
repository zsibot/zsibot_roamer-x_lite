...
: navigo_util::LifecycleNode("planner_server", "", options),
  gp_loader_("navigo_core", "navigo_core::GlobalPlanner"),
  default_ids_{"GridBased"},
  default_types_{"navigo_navfn_planner/NavfnPlanner"},
  costmap_(nullptr)
{
...
PlannerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  for (size_t i = 0; i != planner_ids_.size(); i++) {
    try {
      planner_types_[i] = navigo_util::get_plugin_type_param(
        node, planner_ids_[i]);
      navigo_core::GlobalPlanner::Ptr planner =
  ...
  std::shared_ptr<nav2_msgs::srv::IsPathValid::Response> response)
{
  ...
  if (costmap_ros_->getRobotPose(current_pose)) {
    ...
    for (unsigned int i = 0; i < request->path.poses.size(); ++i) {
      ...
      current_distance = navigo_util::geometry_utils::euclidean_distance(
        current_point,
        path_point);
     ...
     */
    std::unique_lock<navigo_costmap_2d::Costmap2D::mutex_t> lock(*(costmap_->getMutex()));
    unsigned int mx = 0;
    unsigned int my = 0;
    for (unsigned int i = closest_point_index; i < request->path.poses.size(); ++i) {
      ...
      if (cost == navigo_costmap_2d::LETHAL_OBSTACLE ||
        cost == navigo_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
      {
        response->is_valid = false;
      }
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(navigo_path_planner::PlannerServer)
