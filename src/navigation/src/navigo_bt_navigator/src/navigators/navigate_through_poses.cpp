  ...
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  ...
  if (!node->has_parameter("default_nav_through_poses_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("navigo_bt_navigator");
...
{
  using namespace navigo_util::geometry_utils;  // NOLINT
  ...
  try {
    ...
    double distance_remaining =
      navigo_util::geometry_utils::calculate_path_length(current_path, find_closest_pose_idx());
