...
#ifndef NAVIGO_BT_NAVIGATOR__NAVIGATOR_HPP_
#define NAVIGO_BT_NAVIGATOR__NAVIGATOR_HPP_
...
{
public:
  using Ptr = std::shared_ptr<navigo_bt_navigator::Navigator<ActionT>>;
  ...
  bool on_configure(
    ...
    navigo_bt_navigator::NavigatorMuxer * plugin_muxer,
    std::shared_ptr<navigo_util::OdomSmoother> odom_smoother)
  {
    ...
    bt_action_server_ = std::make_unique<navigo_behavior_tree::BtActionServer<ActionT>>(
      node,
      getName(),
      plugin_lib_names,
    ...
    blackboard->set<std::shared_ptr<navigo_util::OdomSmoother>>("odom_smoother", odom_smoother);  // NOLINT
   ...
   */
  std::unique_ptr<navigo_behavior_tree::BtActionServer<ActionT>> & getActionServer()
  {
...
protected:
  ...
  void onCompletion(
    ...
    const navigo_behavior_tree::BtStatus final_bt_status)
  {
  ...
  virtual void goalCompleted(
    typename ActionT::Result::SharedPtr result,
    const navigo_behavior_tree::BtStatus final_bt_status) = 0;
  ...
  std::unique_ptr<navigo_behavior_tree::BtActionServer<ActionT>> bt_action_server_;
...
#endif  // NAVIGO_BT_NAVIGATOR__NAVIGATOR_HPP_
