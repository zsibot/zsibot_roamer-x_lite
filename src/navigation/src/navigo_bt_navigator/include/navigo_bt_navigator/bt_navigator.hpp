...
#ifndef NAVIGO_BT_NAVIGATOR__BT_NAVIGATOR_HPP_
#define NAVIGO_BT_NAVIGATOR__BT_NAVIGATOR_HPP_
...
#include "navigo_bt_navigator/navigators/navigate_through_poses.hpp"

namespace navigo_bt_navigator
{

/**
 * @class navigo_bt_navigator::BtNavigator
 * @brief An action server that uses behavior tree for navigating a robot to its
 * goal position.
 */
class BtNavigator : public navigo_util::LifecycleNode
{
public:
  /**
   * @brief A constructor for navigo_bt_navigator::BtNavigator class
   * @param options Additional options to control creation of the node.
   */
  explicit BtNavigator(rclcpp::NodeOptions options = rclcpp::NodeOptions());
  /**
   * @brief A destructor for navigo_bt_navigator::BtNavigator class
   */
  ~BtNavigator();
...
protected:
  ...
  std::unique_ptr<navigo_bt_navigator::Navigator<nav2_msgs::action::NavigateToPose>> pose_navigator_;
  std::unique_ptr<navigo_bt_navigator::Navigator<nav2_msgs::action::NavigateThroughPoses>>
  poses_navigator_;
  navigo_bt_navigator::NavigatorMuxer plugin_muxer_;

  // Odometry smoother object
  std::shared_ptr<navigo_util::OdomSmoother> odom_smoother_;
...
#endif  // NAVIGO_BT_NAVIGATOR__BT_NAVIGATOR_HPP_
