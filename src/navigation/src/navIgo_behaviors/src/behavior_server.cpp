...
#include "navigo_behaviors/behavior_server.hpp"
...
: LifecycleNode("behavior_server", "", options),
  ...
  default_types_{"navigo_behaviors/Spin",
...
BehaviorServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  costmap_sub_ = std::make_unique<navigo_costmap_2d::CostmapSubscriber>(
    shared_from_this(), costmap_topic);
  footprint_sub_ = std::make_unique<navigo_costmap_2d::FootprintSubscriber>(
...
BehaviorServer::loadBehaviorPlugins()
{
  ...
  for (size_t i = 0; i != behavior_ids_.size(); i++) {
    behavior_types_[i] = navigo_util::get_plugin_type_param(node, behavior_ids_[i]);
    try {
      RCLCPP_INFO(
...
BehaviorServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  std::vector<pluginlib::UniquePtr<navigo_core::Behavior>>::iterator iter;
